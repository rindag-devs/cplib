...
例如此处，`Args` 被设定为绑定到命令行参数的结构体的名称．这会在代码的全局命名空间中导出一个名为 `Args` 的结构体，该结构体的成员将会作为命令行参数的 parser，在 initializer 初始化时自动解析命令行参数．该结构体中的所有成员变量都必须为以下两种类型中的一种：

- `Flag`：「标志风格」（`--var`）的 parser．`Flag` 类型的构造函数接收一个 `std::string` 作为命令行参数的名称（即 `--var` 中的 `var` 部分，不包含前面的两个减号）．运行该 parser 时，会检查 `--var` 是否在命令行参数中存在，若存在则设置成员变量 `value` 为 `true`，不存在则为 `false`．
- `Var<T>`：「变量风格」（`--var=<value>`）的 parser，其中 `T` 为一个变量模板的类型（需继承自 `cplib::var::Var` 类）．`Var<T>` 类型的构造函数接收一个 `T` 类型的实例，该实例的 `name()` 方法的返回值将作为命令行参数的名称（即 `--var=<value>` 中的 `var` 部分，不包含前面的两个减号）．运行该 parser 时，会调用 `T` 类型的 `read_from` 函数解析 `<value>` 字符串，并将解析结果放入 `value` 成员变量中．

`Args`、`Var<T>`、`Flag` 三个结构体均定义在一个相同的隐藏命名空间中，而仅有 `Args` 被导出到全局命名空间，故无需担心全局命名污染．

## 使用

Generator 的使用与 Checker 大致相同．只不过使用时无需传递文件参数，而是要根据 `Args` 结构体的定义传递命令行参数．
