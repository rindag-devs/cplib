...
`value` 可以是 `nullptr`、布尔值、整数、浮点数、字符串、`cplib::json::Map`、`cplib::json::List`、`cplib::json::Raw`，或元素能递归转换为 JSON value 的 input range．Range 和惰性 view 会被直接消费；对于 sized range，转换前会按最终长度为 JSON array 预留容量．

```cpp
using cplib::json::Map;

in.attach_tag("answer", 42);
in.attach_tag(
    "hull/graph",
    Map{
        {"name", "graph"},
...
JSON null 使用 `nullptr` 表示．JSON object 需要显式使用 `Map`，以避免嵌套 initializer list 无法区分 object 和 array 的歧义．
