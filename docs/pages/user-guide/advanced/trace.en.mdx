...
`value` can be `nullptr`, a boolean, an integer, a floating-point value, a string, `cplib::json::Map`, `cplib::json::List`, `cplib::json::Raw`, or an input range whose elements are recursively convertible to JSON values. Ranges and lazy views are consumed directly; sized ranges reserve their final JSON array capacity before conversion.
...
JSON null is represented by `nullptr`. Objects use an explicit `Map`; this avoids ambiguous nested initializer-list syntax between JSON objects and arrays.

For the built-in variable read-in templates in CPLib, if the read-in type is a basic type or a string, the `#v` will be added to indicate the value read-in.
