...
The interactor is used to interact with the standard input/output of the participant program in stdio interactive problems.

In CPLib, interactor performs both "interacting with the participant program" and "checking whether the answer is correct", instead of having two separate programs like Testlib or Polygon's design. 

Interactor usually needs to deal with two input streams:

- The input file of the test point (`inf`): contains information about the test point. Note: In interactive problems, this file is **not equivalent** to the standard input of the participant program.
- User input stream (`from_user`): connects to the standard output of the participant program, used to read the output information of the participant.

And one output stream:

- User output stream (`to_user`): used to send data to the standard input of the participant program.
...
1. Interactor reads the test point information from the test point input file (`inf`) and extracts the part that can be known to the participant program, then sends it to the standard input of the participant program through the user output stream (`to_user`) according to the given format.
2. The participant program sends several operations to interactor, which interactor reads and parses through the user input stream (`from_user`). The return value of the operation is sent to the standard input of the participant program through the user output stream (`to_user`). In some problems, an error code should also be sent to indicate whether the participant program should continue to interact or exit the program.
...
- And all the suggestions mentioned in the chapter [Checker](/user-guide/checker#suggestions).
