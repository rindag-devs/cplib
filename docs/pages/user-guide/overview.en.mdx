...
struct Edge {
  ...
  static auto read(cplib::var::Reader &in, std::int32_t n) -> Edge {
    auto [from, to] = in(cplib::var::i32("from", 1, n), cplib::var::i32("to", 1, n));
    if (from == to) {
      in.fail("Self-loops are not allowed");
    }
...
struct Graph {
  ...
  static auto read(cplib::var::Reader &in) -> Graph {
    auto [n, m] = in(cplib::var::i32("n"), cplib::var::i32("m"));
    auto edges = in.read(cplib::var::ExtVar<Edge>("edge", n) * m);
