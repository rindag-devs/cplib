...
<Callout type="info">
  本节的相关代码见 [GitHub
  仓库](https://github.com/rindag-devs/cplib/tree/main/tests/validator/1-undirected-graph)．
</Callout>
...
第二个版本可以为当前 trait 添加依赖．我们可以用一个「name to value」的 `std::map` 描述所有依赖．只有当全部依赖均被求值，且求出的值等于 `value` 时，才会进行对本 trait 的检查；否则，该 trait 将不会被求值．

```cpp
CPLIB_REGISTER_VALIDATOR(Input, traits);
```

类似于 checker，validator 需要调用 `CPLIB_REGISTER_VALIDATOR` 宏将程序注册成一个 CPLib validator．

<Callout type="warning">
  各 trait 之间的依赖关系必须形成一个有向无环图（DAG），否则会在调用 `val.traits` 时引发 panic
  退出程序．
</Callout>
