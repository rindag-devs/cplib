...
For reading collections of custom types where each element's parsing depends on contextual information (like its index or a corresponding element from another collection), CPLib provides `cplib::var::ExtVec<T>`. It iterates over a given range (e.g., `std::views::iota` or another `std::vector`) and, for each element in the range, calls `T::read`, passing the range element as an additional argument. This avoids manual loops and makes the code more declarative. For example, to read `T` test cases where each `TestCaseInput::read` needs its index:

```cpp copy
auto test_cases = in.read(cplib::var::ExtVec<TestCaseInput>(
    "test_cases",
    std::views::iota(0, T),
    cplib::var::Separator(std::nullopt) // No separator between test cases
));
```
