...
When it is found that the participant's output and the standard answer are not consistent, the `chk.quit_wa` function needs to be called to report "Wrong Answer" and exit the program. We recommend providing clear information when using the `chk.quit_wa` series of functions. Here, the `cplib::format` function is used to format the error message. This function uses a format similar to `std::format`, making it convenient to format strings.
