...
在发现选手输出和标准答案不一致时，需要调用 `chk.quit_wa` 函数报告 Wrong Answer 并退出程序．我们推荐你在使用 `chk.quit_wa` 系列函数时给出清晰的信息．此处使用 `cplib::format` 函数对错误信息进行格式化，该函数使用类似 `std::format` 的格式，可便捷地格式化字符串．
