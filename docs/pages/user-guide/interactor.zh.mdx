...
CPLib 中 Interactor 同时担任「与选手程序交互」和「检验答案是否正确」两个功能，而不是像 Testlib/Polygon 的设计那样，两个功能由 Interactor 和 Checker 两个单独的程序实现．
...
if __name__ == "__main__":
    main()
```
其中代码中常量 `BIN_INTR` 和 `BIN_STD` 的值为执行 Interactor 和选手程序所需的命令，通常为编译 Interactor 和选手程序得到的可执行文件的相对路径．例如 Windows 下，应为 `.\intr.exe` 和 `.\std.exe`．
...
- 以及在 [Checker](/user-guide/checker#建议) 一章中提到的所有建议．